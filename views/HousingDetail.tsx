...
import { ArrowLeft, ExternalLink, Home, MapPin, PencilLine, Phone, WalletCards } from 'lucide-react';
...
export default function HousingDetail({ housingId }: { housingId: string }) {
  ...
  return <ContentColumn className="animate-in bg-white pb-24 fade-in duration-500">
    <header className="relative h-72 overflow-hidden bg-foreground">{item.imageUrl ? <img src={item.imageUrl} alt={item.title} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-white/30"><Home size={72} /></div>}<div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/10 to-transparent" /><Link href="/moradia" className="absolute left-4 top-4 flex h-10 w-10 items-center justify-center rounded-full bg-white/90 text-foreground"><ArrowLeft size={18} /></Link>{item.canEdit ? <button type="button" onClick={() => { setDraft(item); setEditing(true); }} className="absolute right-4 top-4 inline-flex h-10 items-center gap-2 rounded-full bg-white/90 px-4 text-xs font-bold text-foreground"><PencilLine size={15} /> Editar</button> : null}<div className="absolute bottom-6 left-5 right-5 text-white"><span className="rounded-full bg-white/15 px-3 py-1 text-xs font-bold uppercase tracking-widest backdrop-blur">{item.propertyType}</span><h1 className="mt-3 text-3xl font-bold">{item.title}</h1></div></header>
    <div className="space-y-7 px-5 pt-8"><section className="grid gap-3 sm:grid-cols-2"><div className="rounded-3xl bg-slate-50 p-4"><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-400"><MapPin size={15} /> Localização</p><p className="mt-2 font-bold">{item.locationLabel}</p></div><div className="rounded-3xl bg-slate-50 p-4"><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-400"><WalletCards size={15} /> Valor</p><p className="mt-2 font-bold">{item.price}</p></div></section><section className="border-t border-border pt-6"><h2 className="text-h3 font-bold">Sobre a moradia</h2><p className="mt-3 whitespace-pre-wrap text-body-sm leading-7 text-muted-foreground">{item.description}</p><p className="mt-4 text-xs text-slate-400">Publicado por {item.createdBy?.name || 'Membro da comunidade'}</p></section>{item.galleryUrls?.length ? <section className="border-t border-border pt-6"><h2 className="text-h3 font-bold">Galeria</h2><div className="mt-4 grid grid-cols-3 gap-2 sm:grid-cols-6">{item.galleryUrls.map((url, index) => <button key={url + index} type="button" onClick={() => setLightboxImage(url)} className="aspect-square overflow-hidden rounded-2xl bg-slate-100"><img src={url} alt={`${item.title} - foto ${index + 1}`} className="h-full w-full object-cover" /></button>)}</div></section> : null}{contactHref ? <a href={contactHref} target={contactIsUrl ? '_blank' : undefined} rel={contactIsUrl ? 'noreferrer' : undefined} className="inline-flex items-center justify-center gap-2 rounded-full bg-brand-500 px-5 py-3 text-sm font-bold text-white">{contactIsUrl ? <ExternalLink size={17} /> : <Phone size={17} />} Entrar em contato</a> : null}</div>
