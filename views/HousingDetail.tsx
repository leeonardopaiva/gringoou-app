...
export default function HousingDetail({ housingId }: { housingId: string }) {
  ...
  return <ContentColumn className="animate-in bg-white pb-24 fade-in duration-500">
    <header className="relative h-72 overflow-hidden bg-foreground">{item.imageUrl ? <img src={item.imageUrl} alt={item.title} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-white/30"><Home size={72} /></div>}<div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/10 to-transparent" /><Link href="/moradia" className="absolute left-4 top-4 flex h-10 w-10 items-center justify-center rounded-full bg-white/90 text-foreground"><ArrowLeft size={18} /></Link>{item.canEdit ? <Dropdown align="right" className="absolute right-4 top-4" trigger={<span className="flex h-10 w-10 items-center justify-center rounded-full bg-white/90 text-slate-700 shadow-sm"><MoreHorizontal size={20} /></span>} sections={[{ heading: 'Ações da moradia', items: [{ label: 'Editar moradia', icon: <PencilLine size={16} />, onClick: () => { setDraft(item); setEditing(true); } }] }]} /> : null}<div className="absolute bottom-6 left-5 right-5 text-white"><span className="rounded-full bg-white/15 px-3 py-1 text-xs font-bold uppercase tracking-widest backdrop-blur">{item.propertyType}</span><h1 className="mt-3 text-3xl font-bold">{item.title}</h1></div></header>
