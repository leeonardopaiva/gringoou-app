...
const MAX_LIMIT = 12;
const CATEGORIES = ['all', 'businesses', 'events', 'posts', 'people', 'groups', 'jobs', 'interests'] as const;
...
export async function GET(request: Request) {
  ...
  const [businessResult, eventResult, postResult, peopleResult, groupResult, jobResult, interestUsers] = await Promise.all([
    wantsCategory(category, 'businesses') ? Promise.all([
      prisma.business.findMany({ where: businessWhere, orderBy: [{ ratingCount: 'desc' }, { createdAt: 'desc' }], skip, take: limit, select: { id: true, slug: true, name: true, category: true, address: true, description: true, imageUrl: true, locationLabel: true, ratingAverage: true, ratingCount: true } }),
      prisma.business.count({ where: businessWhere }),
    ]) : emptyPage<never>(),
    wantsCategory(category, 'events') ? Promise.all([
      prisma.event.findMany({ where: eventWhere, orderBy: [{ startsAt: 'asc' }], skip, take: limit, select: { id: true, slug: true, title: true, venueName: true, startsAt: true, locationLabel: true, description: true, imageUrl: true, ratingAverage: true, ratingCount: true } }),
  ...
  const counts = { businesses: businessCount, events: eventCount, posts: postCount, people: peopleCount, groups: groupCount, jobs: jobCount, interests: matchingInterests.length };
  const selectedCount = category === 'all' ? Math.max(...Object.values(counts)) : counts[category];

  return NextResponse.json({
    query, category, page, pageSize: limit,
    filters: { region: viewerRegionKey || '', country, city, businessType },
    businesses, events, people, groups, jobs, interests,
