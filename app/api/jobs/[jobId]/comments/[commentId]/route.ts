...
type Context = { params: Promise<{ jobId: string; commentId: string }> };
...
async function getAccess(jobId: string, commentId: string, session: Awaited<ReturnType<typeof getServerAuthSession>>) {
  const job = await prisma.job.findFirst({ where: { id: jobId, isActive: true }, select: { id: true, createdById: true } });
  if (!job) return null;
  const comment = await prisma.jobComment.findFirst({ where: { id: commentId, jobId }, select: { id: true, authorId: true } });
  if (!comment) return null;
  const manage = canManage(session, job.createdById);
...
export async function PUT(request: Request, context: Context) {
  ...
  const { jobId, commentId } = await context.params;
  const access = await getAccess(jobId, commentId, session);
  ...
  const comment = await prisma.jobComment.update({ where: { id: commentId }, data: { content: parsed.data.content }, include: { author: { select: authorSelect } } });
...
export async function PATCH(request: Request, context: Context) {
  ...
  const comment = await prisma.jobComment.update({ where: { id: commentId }, data: { isHidden: hide, hiddenAt: hide ? new Date() : null }, include: { author: { select: authorSelect } } });
